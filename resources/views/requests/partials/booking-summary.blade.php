<aside class="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-800 dark:bg-emerald-950/20" aria-labelledby="booking-summary-heading">
    ...
    <div class="mt-3 space-y-2 text-sm">
        <template x-for="schedule in dailySchedules" :key="`summary-${schedule.date}`">
            <div class="flex flex-wrap justify-between gap-2 border-b border-emerald-900/10 pb-2">
                ...
                <span x-show="schedule.start && schedule.end">
                    <strong x-text="`${formatTime(schedule.start)}–${formatTime(schedule.end)}`"></strong>
                    · <span x-text="`${duration(schedule) / 60} hour${duration(schedule) === 60 ? '' : 's'}`"></span>
                </span>
                <span x-show="!schedule.start || !schedule.end" class="font-semibold text-amber-700 dark:text-amber-300">Select start and end times</span>
            </div>
        </template>
    </div>
    <div class="mt-3 flex items-center justify-between gap-3 text-sm font-semibold">
        ...
        <span
            ...
            :class="!sharedStartTime || !sharedEndTime ? 'bg-zinc-100 text-zinc-600' : hasBlockingConflict() ? 'bg-red-100 text-red-700' : hasPendingWarning() ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-700'"
            x-text="!sharedStartTime || !sharedEndTime ? 'Choose a time' : availabilityLoading ? 'Checking…' : hasApprovedConflict() ? 'Unavailable — Already booked' : hasClosure() ? 'Unavailable — Closed or under maintenance' : hasPendingWarning() ? 'Pending request exists' : 'Available'"
        ></span>
    </div>
