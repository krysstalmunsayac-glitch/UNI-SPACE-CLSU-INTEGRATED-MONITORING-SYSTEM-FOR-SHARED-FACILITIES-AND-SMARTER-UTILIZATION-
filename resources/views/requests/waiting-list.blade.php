                        ...
                        <form action="{{ route('waiting.list.update', $request) }}" method="POST" class="mt-6 space-y-4">
                            ...
                            <div class="grid gap-4 lg:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <input id="Proposed_Date_{{ $request->RID }}" name="Proposed_Date" type="date" min="{{ app(\App\Services\BookingPolicy::class)->earliestDate(auth()->user()) }}" value="{{ old('Proposed_Date', $request->Proposed_Date?->toDateString()) }}" class="w-full rounded-xl border border-emerald-900/10 bg-white px-3 py-2 text-sm shadow-sm focus:border-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-600/10 dark:border-white/10 dark:bg-zinc-900" />
                                </div>
                                <div>
                                ...
                                <div>
                                    <label class="mb-2 block text-sm font-medium text-emerald-900 dark:text-zinc-300" id="Proposed_Start_Time_{{ $request->RID }}-label" for="Proposed_Start_Time_{{ $request->RID }}">Start time</label>
                                    <x-ui::time-select id="Proposed_Start_Time_{{ $request->RID }}" aria-labelledby="Proposed_Start_Time_{{ $request->RID }}-label" name="Proposed_Start_Time" :value="old('Proposed_Start_Time', $request->Proposed_Start_Time?->format('H:i'))" :options="app(\App\Services\FacilityAvailabilityService::class)->slots()" />
                                </div>
                                <div>
