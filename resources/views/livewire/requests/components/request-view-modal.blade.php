        ...
        <article class="mx-auto w-full max-w-5xl bg-white px-5 py-6 text-zinc-900 shadow-xl ring-1 ring-black/5 dark:bg-zinc-950 dark:text-white sm:px-8 sm:py-8">
                ...
                <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
                    ...
                    <dl class="grid min-w-0 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd class="font-medium">{{ $Is_Guest_Booking ? 'No End User account' : ($form->User_ID ? '#'.$form->User_ID : '—') }}</dd>
                        </div>
                        <div>
            ...
            <section class="mt-6">
                ...
                <dl class="grid overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50/60 sm:grid-cols-2 dark:border-zinc-800 dark:bg-zinc-900/50">
                    @foreach ([
                        'Date range' => $form->Proposed_Date ? \Carbon\Carbon::parse($form->Proposed_Date)->format('M d, Y').($form->Proposed_End_Date && $form->Proposed_End_Date !== $form->Proposed_Date ? ' – '.\Carbon\Carbon::parse($form->Proposed_End_Date)->format('M d, Y') : '') : '—',
                        'Time' => ($form->Proposed_Start_Time ?: '—').' – '.($form->Proposed_End_Time ?: '—'),
                        'Facility' => $Facility_Name ?? '—',
                        'Facility location' => $Facility_Location ?: '—',
                        'Managing office' => $Facility_Office ?: '—',
                        'Event' => $Event_Title ?? '—',
                        'Event type' => $Event_Type ?: '—',
                        'Expected Attendees' => $form->Capacity ?? '—',
                    ] as $label => $value)
