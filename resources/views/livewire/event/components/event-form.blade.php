            <x-ui::card>
                ...
                <form action="{{ route('events.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        <x-ui::input label="Event title" name="Event_Title" value="{{ old('Event_Title') }}" />
                        @error('Event_Title') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <x-ui::textarea label="Description" name="Description" rows="4">{{ old('Description') }}</x-ui::textarea>
                        @error('Description') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <x-ui::input label="Event type" name="Type_Event" value="{{ old('Type_Event') }}" placeholder="e.g. Workshop, Seminar, Social" />
                        @error('Type_Event') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <x-ui::button type="submit" class="w-full sm:w-auto">Create event</x-ui::button>
